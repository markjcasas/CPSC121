...
public:
  void set_source_route(std::string source_route) {source_route_ = source_route; }
  void set_destination_route(std::string destination_route) { destination_route_ = destination_route; }
  std::string source_route() { return source_route_; }
  std::string destination_route() { return destination_route_; }
  ConnectionRoute() : ConnectionRoute("Walk to bus", "Home", "Bus stop", 800, 805) {}
  ConnectionRoute(std::string name, std::string source_route, std::string destination_route, int departure_time, int arrival_time)
      : Route(name, departure_time, arrival_time), source_route_(source_route), destination_route_(destination_route) {}
  void display() override;
  std::string save_string() override;
};
class RouteManager {
