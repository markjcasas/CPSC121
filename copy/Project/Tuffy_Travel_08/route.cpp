...
void ConnectionRoute::display() {
  std::cout << "Connection Route: " << name() << "\nSource: " << source_route_
  ...
  if (infile.is_open()) {
    ...
    for (int load_count = 0; load_count < load_size; load_count++) {
      ...
      if (ifstream_string == "[Route]") {
        Route* ifstream_route = new Route;
        std::getline(infile, ifstream_string);
        ...
        add(ifstream_route);
        infile.seekg(1, infile.cur);
      } else if (ifstream_string == "[CheckedRoute]") {
        CheckedRoute* ifstream_checked_route = new CheckedRoute;
        std::getline(infile, ifstream_string);
        ...
        ConnectionRoute* ifstream_connection_route = new ConnectionRoute;
        std::getline(infile, ifstream_string);
        ifstream_connection_route->set_name(ifstream_string);
        std::getline(infile, ifstream_string);
        ifstream_connection_route->set_source_route(ifstream_string);
        std::getline(infile, ifstream_string);
        ifstream_connection_route->set_destination_route(ifstream_string);
        infile >> ifstream_int;
        ifstream_connection_route->set_departure_time(ifstream_int);
        infile >> ifstream_int;
        ifstream_connection_route->set_arrival_time(ifstream_int);
        add(ifstream_connection_route);
        infile.seekg(1, infile.cur);
      }
    }
    infile.close();
...
ConnectionRoute* create_connection_route() {
  ConnectionRoute* new_connection_route = new ConnectionRoute;
  std::string new_connection_route_name;
  std::string new_connection_route_source_route;
  std::string new_connection_route_destination_route;
  int new_connection_route_departure_time;
  int new_connection_route_arrival_time;
  std::cout << "Please enter the name of the connection route: ";
  std::getline(std::cin, new_connection_route_name);
  new_connection_route->set_name(new_connection_route_name);
  std::cout << "Please enter the source route: ";
  std::getline(std::cin, new_connection_route_source_route);
  new_connection_route->set_source_route(new_connection_route_source_route);
  std::cout << "Please enter the destination route: ";
  std::getline(std::cin, new_connection_route_destination_route);
  new_connection_route->set_destination_route(new_connection_route_destination_route);
  std::cout << "Please enter the departure time of the connection route: ";
  std::cin >> new_connection_route_departure_time;
  new_connection_route->set_departure_time(new_connection_route_departure_time);
  std::cout << "Please enter the arrival time of the connection route: ";
  std::cin >> new_connection_route_arrival_time;
  new_connection_route->set_arrival_time(new_connection_route_arrival_time);
  std::cin.ignore();
  return new_connection_route;
}
...
int tuffy_travel(RouteManager tuffy_travel_object) {
  ...
  if (decision == 'R' || decision == 'r') {
    ...
    std::cout << '\n';
    tuffy_travel_object.add(create_connection_route());
    return tuffy_travel(tuffy_travel_object);
