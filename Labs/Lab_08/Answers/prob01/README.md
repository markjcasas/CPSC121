# Count Odd
Create a program that counts all the odd numbers from 0 to a number using recursion. Your program should have a function called `count_odd` that takes in an `int` value
and returns an `int` value. The user will be asked for a number, and the program will display the following output.
